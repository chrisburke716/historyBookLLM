...
async def demonstrate_chat_service():
    ...
    try:
        ...
        search_results = await chat_service.search_messages(
            query="alliance system",
            session_id=session.id,
            limit=3
        )
        print(f"   🎯 Found {len(search_results)} messages mentioning 'alliance system'")
        ...
        if ai_messages:
            avg_response_length = sum(len(m.content) for m in ai_messages) / len(ai_messages)
