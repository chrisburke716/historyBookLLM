...
def clear_database(config: WeaviateConfig, collection_types: list = None):
    ...
    if collection_types is None:
        collection_types = ["paragraphs", "chapters", "books"]  # Order matters for dependencies
    ...
    try:
        ...
        if samples["books"]:
            ...
            for book in samples["books"][:3]:
                logger.info(f"  - {book.title} (pages {book.start_page}-{book.end_page})")
        ...
        if total_entities > 0:
            ...
            if clear_choice == "yes":
                ...
                if final_total == 0:
                    logger.info("✅ Database is now empty and ready for fresh ingestion!")
                else:
