...
def main():
    ...
    if success:
        ...
        logger.info("  - Test search: python -c \"from history_book.database.repositories import BookRepositoryManager; from history_book.database.config import WeaviateConfig; manager = BookRepositoryManager(WeaviateConfig.from_env()); print(manager.paragraphs.similarity_search_by_text('test', limit=1))\"")
    else:
