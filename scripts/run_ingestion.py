...
def main():
    ...
    try:
        ...
        logger.info(f"\n📈 Summary:")
        logger.info(f"  Average chapters per book: {len(chapter_ids) / len(book_ids):.1f}")
        logger.info(f"  Average paragraphs per chapter: {len(paragraph_ids) / len(chapter_ids):.1f}")
        logger.info(f"  Total entities created: {len(book_ids) + len(chapter_ids) + len(paragraph_ids)}")
        
        logger.info(f"\n✨ New repository pattern benefits demonstrated:")
        logger.info(f"  ✅ Pure entity creation (no auto-saving)")
        logger.info(f"  ✅ Explicit database operations")
        logger.info(f"  ✅ Batch operations for efficiency")
        logger.info(f"  ✅ Proper error handling")
        logger.info(f"  ✅ Clean separation of concerns")
