...
def get_chapter_titles_from_ToC(doc: pymupdf.Document) -> tuple[list[str], list[list[str]]]:
    """
...
def get_chapter_text_blocks(doc: pymupdf.Document, start_page: int, end_page: int) -> tuple[list[str], list[int]]:
    """
...
def concatenate_paragraphs(paragraphs: list[str], page_numbers: list[int]) -> tuple[list[str], list[int]]:
    """
...
def create_paragraphs_db(
    ...
    for i, (block, page) in enumerate(zip(text_blocks, page_numbers), start=start_index):
...
def process_chapter(
    ...
    concatenated_blocks, concatenated_pages = concatenate_paragraphs(blocks, block_pages)
    ...
    paragraphs = create_paragraphs_db(concatenated_blocks, concatenated_pages, chapter_index, book_index)
...
def process_book(
    ...
    end_page = book_start_pages[book_index + 1][0] + 1  # Next book's start page (1-indexed)
...
def build_history_book_db() -> tuple[list[BookDBModel], list[ChapterDBModel], list[ParagraphDBModel]]:
    """
    ...
    for book_index, (title, chapter_titles) in enumerate(zip(book_titles, chapters_by_book)):
        # Process this book
