...
def pydantic_field_to_weaviate_property(field_name: str, field_info: FieldInfo) -> Optional[Property]:
...
def create_collection_from_pydantic(client: WeaviateClient, model_class: type[BaseModel], class_name: str) -> Collection:
    model_fields = model_class.model_fields
    properties = [
        prop for field_name, field_info in model_fields.items()
        if (prop := pydantic_field_to_weaviate_property(field_name, field_info)) is not None
    ]
