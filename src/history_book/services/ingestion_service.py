...
class IngestionService:
    ...
    def clear_all_data(self) -> dict:
        ...
        try:
            ...
            logger.info(f"✅ Successfully cleared {total_deleted} entities from database")
        ...
        try:
            ...
            if total_existing > 0:
                ...
                if clear_existing:
                    ...
                    logger.warning("Consider using clear_existing=True or running the clear script first.")
            ...
            for book_index, (book_title, chapter_titles) in enumerate(zip(book_titles, chapters_by_book)):
                book_entity, chapter_entities, paragraph_entities = self._process_book_to_entities(
                ...
                logger.info(f"Processed book {book_index + 1}/{len(book_titles)}: {book_title}")
                logger.info(f"  Chapters: {len(chapter_entities)}, Paragraphs: {len(paragraph_entities)}")
            ...
            logger.info(f"Successfully ingested book with {len(book_ids)} books, {len(chapter_ids)} chapters, {len(paragraph_ids)} paragraphs")
    ...
    def _get_chapter_starts(self, doc: pymupdf.Document, final_page: int) -> List[List[int]]:
        """
    ...
    def _get_chapter_titles_from_toc(self, doc: pymupdf.Document) -> Tuple[List[str], List[List[str]]]:
        """
    ...
    def _organize_chapter_titles_by_book(self, chapter_titles: List[str]) -> List[List[str]]:
        """
    ...
    def _process_book_to_entities(
        ...
        for i, (chapter_title, chapter_page) in enumerate(zip(chapter_titles, chapter_pages)):
            chapter_start = chapter_page + 1
            chapter_end = chapter_pages[i + 1] + 1 if i < len(chapter_pages) - 1 else end_page
    ...
    def _extract_chapter_paragraphs(
        ...
        concatenated_blocks, concatenated_pages = self._concatenate_paragraphs(blocks, block_pages)
        
        # Create paragraph entities
        paragraph_entities = []
        for i, (text_block, page) in enumerate(zip(concatenated_blocks, concatenated_pages)):
            paragraph = Paragraph(
                text=text_block,
                page=page,
                paragraph_index=i,
    ...
    def _get_chapter_text_blocks(self, doc: pymupdf.Document, start_page: int, end_page: int) -> Tuple[List[str], List[int]]:
                ...
                for block in page_blocks:
                    if block[6] == 0:  # Text blocks only
                        ...
                        if text and len(text) > 10:  # Filter out very short blocks
                            blocks.append(text)
                            block_pages.append(page_num + 1)  # Convert back to 1-indexed
        
        return blocks, block_pages
    
    def _concatenate_paragraphs(self, blocks: List[str], block_pages: List[int]) -> Tuple[List[str], List[int]]:
        """
