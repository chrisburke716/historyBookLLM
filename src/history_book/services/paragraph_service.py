...
class ParagraphService:
    ...
    def create_paragraph(self, paragraph: Paragraph) -> str:
        ...
        try:
            ...
            if created_paragraph:
                ...
                logger.debug(
                    f"Created paragraph {paragraph_id} with embedding of {len(vector) if vector else 0} dimensions"
                )
            else:
                logger.warning(
                    f"Created paragraph {paragraph_id} but couldn't fetch it back"
                )
    ...
    def search_similar_paragraphs(
        ...
        try:
            ...
            logger.debug(
                f"Found {len(results)} similar paragraphs for query: '{query_text[:50]}...'"
            )
            return results
    ...
    def batch_create_paragraphs(self, paragraphs: list[Paragraph]) -> list[str]:
        ...
        try:
            ...
            entities_and_vectors = [
                (p, None) for p in paragraphs
            ]  # Let Weaviate generate vectors

            # Perform batch creation
            paragraph_ids = self.repository.batch_create_with_vectors(
                entities_and_vectors
            )
            ...
            for paragraph, paragraph_id in zip(paragraphs, paragraph_ids, strict=False):
                ...
                try:
                    ...
                    paragraph.embedding = vector
                    logger.debug(
                        f"Updated paragraph {paragraph_id} with embedding of {len(vector) if vector else 0} dimensions"
                    )
                except Exception as e:
                    logger.warning(
                        f"Could not fetch embedding for paragraph {paragraph_id}: {e}"
                    )
