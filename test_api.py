            ...
            if response.status_code == 200:
                ...
                if message.get('citations'):
                    ...
                    if citation_format_ok:
                        ...
                        print(f"   ⚠️  Citation format unexpected: {message['citations']}")
...
def run_full_test_suite():
    ...
    try:
        ...
        print("   💡 Start server with: PYTHONPATH=src poetry run uvicorn src.history_book.api.main:app --reload --port 8000")
    ...
    if server_available:
        ...
        print(f"🌐 API Endpoints:     {'✅ PASS' if all(api_tests_passed) else '❌ FAIL'} ({passed_count}/{total_count})")
    else:
        print(f"🌐 API Endpoints:     ⏸️  SKIPPED (server not running)")
    
    overall_success = models_ok and backend_ok and (not server_available or all(api_tests_passed))
